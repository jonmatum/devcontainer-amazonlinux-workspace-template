...
if [[ "$AUTOSUGGESTIONS" == "true" ]]; then
  ...
  if [ ! -d "$PLUGIN_DIR" ]; then
    ...
    sudo -u "$USERNAME" git clone https://github.com/zsh-users/zsh-autosuggestions "$PLUGIN_DIR"
  fi
...
if [[ "$SYNTAXHIGHLIGHTING" == "true" ]]; then
  ...
  if [ ! -d "$PLUGIN_DIR" ]; then
    ...
    sudo -u "$USERNAME" git clone https://github.com/zsh-users/zsh-syntax-highlighting.git "$PLUGIN_DIR"
  fi
